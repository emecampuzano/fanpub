...
from undetected_playwright import stealth_sync, stealth_async
...
class Sailor:
    ...
    def getDetails(self):
        ...
        self.author = self.page.locator('.author-info__username a').first.text_content()
        self.cover = self.page.locator('.story-cover img').get_attribute('src')
        self.chapters = self.page.query_selector_all('.story-parts >> ul >> li >> a')
        return self 
    
    def createStory(self):
        self.story = Story(self.title, self.author, self.cover, configurator=self.config)
        return self
    ...
    def extractChapters(self):
        for chapter in self.chapterLinks:
            ...
            self.simulateRead()
            self.story.addChapter(self.chapterNames[self.chapterLinks.index(chapter)], self.getParagraphs())
