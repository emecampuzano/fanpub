...
import datetime, os, re, time, warnings
...
class Press: 
    ...
    def __init__(self, story, url, configurator):
        ...
        # Add table of contents
        self.book.toc = ([epub.Link('Table of Contents.xhtml', 'Table of Contents', 'Table of Contents')] + self.book.toc)
    ...
    def processChapter(self, chapter):
        try:
            with open(os.path.join(self.config.content, chapter), 'r', encoding='utf-8') as f:
                ...
                chapter = epub.EpubHtml(title=title, file_name=f'{chapter_number}_{title}.xhtml', lang='en')
    ...
    def addCover(self):
        CoverArt(self.cover).fetch()
        time.sleep(1)
        self.book.set_cover('cover.jpg', open(os.path.join(self.config.media, 'cover.jpg'), 'rb').read())
        
    def insertCover(self):
        cover_page = epub.EpubImage()
        cover_page.file_name = 'cover.jpg'
        cover_page.media_type = 'image/jpeg'
        cover_page.content = open(os.path.join(self.config.media,'cover.jpg'), 'rb').read()
    ...
    def insertDisclaimer(self):
        ...
        disclaimer_page = epub.EpubHtml(title='Disclaimer', file_name='disclaimer.xhtml', lang='en')
        ...
        self.book.spine.insert(2, disclaimer_page)  # Insert after the title pagee
    ...
    def addMetadata(self):
        ...
        self.book.set_unique_metadata('DC', 'date', str(datetime.datetime.now()), {'event': 'publication'})

        # Set publisher
        self.book.set_unique_metadata('DC', 'publisher', 'The Wattpad Thief', {})
